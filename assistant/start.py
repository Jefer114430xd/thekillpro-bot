...
**Reenvio de mensajes** - {udB.get_key("PMBOT")}

**TheKillPro [v{ultroid_version}], powered by @TheKillPro**
"""
...
_settings = [
    [
        ...
        Button.inline("SMS Bot", data="cbs_chatbot"),
    ],
    [
        Button.inline("Aʟɪᴠᴇ", data="cbs_alvcstm"),
        Button.inline("SMS Privado", data="cbs_ppmset"),
    ],
    [
        Button.inline("Caracteristicas", data="cbs_otvars"),
        Button.inline("Ajustes opcionales", data="cbs_vcb"),
    ],
...
_start = [
    [
        ...
        Button.inline("Ajustes ⚙️", data="setter"),
    ],
    [
        Button.inline("Estadisticas ✨", data="stat"),
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ...
        if not udB.get_key("STARTMSG"):
            if udB.get_key("PMBOT"):
                ok = "Envia tu mensaje te respondere luego."
            await event.reply(
